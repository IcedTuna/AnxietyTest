...
function createButtons() {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    img.src = `button/${i + 7}.png`; // 7.png, 8.png, 9.png, 10.png, 11.png
    img.style.width = '50px';
    img.style.height = '50px';
    img.style.cursor = 'pointer';

    img.addEventListener('click', () => {
      buttons.forEach(b => b.src = `button/${parseInt(b.dataset.value) + 6}.png`); // quay lại ảnh thường
      img.src = 'button/6.png'; // ảnh khi chọn
...
function createDots() {
  ...
  pages.forEach((_, index) => {
    ...
    dot.style.backgroundColor = index === current ? "#116f59" : "#ccc";
    dot.onclick = () => { current = index; showPage(); };
